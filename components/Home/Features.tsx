...
const Features: React.FC<FeaturesProps> = ({ activeModule = 'software' }) => {
  ...
  return (
    <section className="py-24 bg-[#0B0E14] relative overflow-hidden">
      {/* Decorative Glow */}
      <div 
        className="absolute top-0 left-1/2 -translate-x-1/2 w-[1000px] h-[600px] blur-[120px] opacity-10 pointer-events-none transition-colors duration-700"
        style={{ backgroundColor: themeColor }}
      />

      <div className="max-w-7xl mx-auto px-4 relative z-10">
        <div className="mb-20 text-center">
          <h2 className="font-tech tracking-widest mb-4 uppercase" style={{ color: themeColor }}>
        ...
        <div className="space-y-40">
          {pillars.map((layer, idx) => {
            ...
            return (
              <div key={`${activeModule}-${idx}`} className={`flex flex-col ${layer.isImageLeft ? 'md:flex-row' : 'md:flex-row-reverse'} items-center gap-20 animate-in fade-in slide-in-from-bottom-10 duration-700`}>
                <div className="flex-1 space-y-8">
                  <div 
                    className="inline-block px-4 py-1 rounded-full border text-xs font-bold uppercase tracking-widest transition-colors duration-700"
                  ...
                  <div>
                    ...
                    <h4 className="text-xl font-medium opacity-80" style={{ color: themeColor }}>{layer.subtitle}</h4>
                  </div>
                  ...
                  {isBatteryPillar && (
                    <div className="grid grid-cols-2 gap-4 pt-4">
                      <div className="glass p-4 rounded-2xl border border-white/5">
                        <div className="text-violet-400 font-tech font-bold text-2xl">30 Days</div>
                        <div className="text-[10px] text-gray-500 uppercase tracking-widest mt-1">Standby Time</div>
                      </div>
                      <div className="glass p-4 rounded-2xl border border-white/5">
                        <div className="text-violet-400 font-tech font-bold text-2xl">15 Hours</div>
                        <div className="text-[10px] text-gray-500 uppercase tracking-widest mt-1">Continuous Rec</div>
                      </div>
                    </div>
                  )}
