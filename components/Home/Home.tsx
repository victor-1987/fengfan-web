...
const Home: React.FC<HomeProps> = ({ onConsult, onBookDemo }) => {
  ...
  return (
    <div className="animate-in fade-in duration-700">
      ...
      <div className="bg-[#0B0E14] pt-24 pb-8">
        <div className="max-w-7xl mx-auto px-4 flex justify-center">
          <div className="inline-flex p-1 bg-white/5 rounded-full border border-white/10 backdrop-blur-md">
            <button 
              ...
              className={`px-10 py-3 rounded-full text-sm font-bold transition-all duration-300 ${activeModule === 'software' ? 'bg-[#2D7FF9] text-white shadow-lg shadow-blue-500/20' : 'text-gray-400 hover:text-white'}`}
            >
              软件服务 (SaaS)
            </button>
            <button 
              onClick={() => setActiveModule('hardware')}
              className={`px-10 py-3 rounded-full text-sm font-bold transition-all duration-300 ${activeModule === 'hardware' ? 'bg-violet-600 text-white shadow-lg shadow-violet-500/20' : 'text-gray-400 hover:text-white'}`}
            >
              AI 智能硬件 (Device)
            </button>
          </div>
